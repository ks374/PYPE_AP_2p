...
>""" % (self.filename, self.header.vers, self.header.freq,
        ...
        len(list(self.trialdata.keys())), self.neurons)
    ...
    def load_plx(self, filename):
        ...
        self.neurons = list(self.channellist.keys())
        sys.stderr.write('\n')
        sys.stderr.flush()
...
def FindTimestampFile(pypefile, search=None):
    ...
    if search is None and 'PLXPATH' in os.environ:
