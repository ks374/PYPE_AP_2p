...
class ParamTable:
	def __init__(self, parent, table, file=None, altfile=None):
		...
		for slot in self._table:
			...
			elif None and type(validate) is tuple:
	...
	def get(self, evaluate=1, mergewith=None):
		...
		for slot in self._table:
			...
			v = self.query(name)
			if evaluate:
				if not (type(validate) is tuple) and validate:
	...
	def save(self, file=None):
		...
		f = open(file, 'wb')   # w-->wb in python3. Feb 2023. 
		pickle.dump(x, f)
		f.close()
	...
	def _load(self, file=None):
		...
		try:
			f = open(file, 'rb')   # r --> rb in python3. Feb 2023
			x = pickle.load(f)
			f.close()
			for slot in self._table:
				(name, default, validate, descr, runlock) = _unpack_slot(slot)
