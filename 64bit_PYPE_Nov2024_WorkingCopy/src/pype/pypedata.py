...
import sys, types, string, _thread, os
#import math, Numeric, MLab, time
import math, numpy, MLab, time
...
class PypeRecord:
	...
	def compute(self, velocity=None, gaps=None, raw=None, nooffset=None):
		...
		if not self.computed:
			...
			self.eyet = numpy.array(self.rec[3], 'f') # ms
			dt = diff(self.eyet)
			if sum(numpy.where(numpy.less(dt, 0), 1, 0)) > 0:
				raise PypedataTimeError
			self.realt = numpy.array(self.rec[3], 'f') # ms
			self.eyex = numpy.array(self.rec[4], 'f') # dva
			self.eyey = numpy.array(self.rec[5], 'f')	# dva
			if lag > 0:
			...
			self.eyevalid = None
			self.photo_times = numpy.array(self.rec[6], 'i')
			self.spike_times = numpy.array(self.rec[7], 'i')
			...
			if len(self.rec) > 13 and self.rec[13] is not None:
				...
				self.plex_times = numpy.array(times, 'i')
				self.plex_channels = numpy.array(channels, 'i')
				self.plex_units = numpy.array(units, 'i')
				self.plex_ids = ids[:]
			else:
			...
			if gaps:
				gaps = numpy.nonzero(numpy.where(numpy.greater(dt, 1),
													 1,0))
				self.gaps_t = numpy.take(self.eyet, gaps)
				self.gaps_y = self.gaps_t * 0
				self.gapdurs = numpy.take(self.eyet, gaps+1) - self.gaps_t
			else:
				self.gaps_t = None
				self.gaps_y = None
...
class PypeFile:
	...
	def backload(self):
		self.loading = 1
		_thread.start_new_thread(self._backload2, (None,))
...
def findfix(d, thresh=2, dur=50, anneal=10, start=None, stop=None):
	...
	for i in range(0, len(fix_ix), 2):
		a = fix_ix[i]
		b = fix_ix[i+1]
		xp = numpy.sum(d.eyex[a:b]) / float(len(d.eyex[a:b]))
		yp = numpy.sum(d.eyey[a:b]) / float(len(d.eyey[a:b]))
