...
class _BLT(Widget):
	...
	def crosshairs_configure(self, cnf={}, **kw):
		return self.tk.call(*(self._w, 'crosshairs', 'configure')
	...
	def element_closest(self, x, y, cnf={}, name=None):
		return self.tk.call(*(self._w, 'element', 'closest', x, y)+
					 self._options(cnf)+(name))

	def element_configure(self, name, cnf={}, **kw):
		self.tk.call(*(self._w, 'element', 'configure', name)
		      + self._options(cnf, kw))

	def element_create(self, name, cnf={}, **kw):
		self.tk.call(*(self._w, 'element', 'create', name)
		      + self._options(cnf, kw))

	def element_deactivate(self, name, *names):
		self.tk.call(*(self._w, 'element', 'deactivate',
				    name) + names)

	def element_delete(self, name, *names):
		self.tk.call(*(self._w, 'element', 'delete',
				    name) + names)
	...
	def element_show(self, *names):
		return self.tk.call(*(self._w, 'element', 'show') + names)
	...
	def legend_activate(self, name, *names):
		self.tk.call(*(self._w, 'legend', 'activate', name) + names)

	def legend_configure(self, cnf={}, **kw):
		self.tk.call(*(self._w, 'legend', 'configure')
		      + self._options(cnf, kw))
	...
	def postscript(self, filename=None):
		...
		if filename == None:
			filename = tkinter.filedialog.SaveAs(initialdir=os.getcwd(),
	...
	def psconfigure(self, cnf={}, **kw):
		self.tk.call(*(self._w, 'postscript', 'configure')
			  + self._options(cnf, kw))

	def psget(self, cnf={}, **kw):
		return self.tk.call(*(self._w, 'postscript', 'cget')
					 + self._options(cnf, kw))
	...
	def tag_configure(self, tagid, cnf={}, **kw):
		self.tk.call(*(self._w, 'tag', 'configure', name)
		      + self._options(cnf, kw))
	...
	def tag_delete(self, tagid, *tagids):
		self.tk.call(*(self._w, 'tag', 'delete',
				    tag) + tags)
	...
	def axis_configure(self, axis, cnf={}, **kw):
		self.tk.call(*(self._w, axis, 'configure')
		      + self._options(cnf, kw))
	...
	def image_create(self, cnf={}, **kw):
		self.tk.call(*(self._w, 'marker', 'create',  'image')
		      + self._options(cnf, kw))

	def marker_create(self, mtype, cnf={}, **kw):
		self.tk.call(*(self._w, 'marker', 'create', mtype)
		      + self._options(cnf, kw))
...
class Histogram(_BLT):
	...
	def display(self, bargraph, name, percent=0, cnf={}, **kw):
		...
		try:
			bargraph.element_configure(*(name,), **kw)
		except:
			bargraph.element_create(*(name,), **kw)
