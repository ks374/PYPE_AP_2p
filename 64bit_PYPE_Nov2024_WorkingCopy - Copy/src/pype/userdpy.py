...
- Mon Nov 26 16:37:00 2007 Phil
  ...
  - added note_at_location to allow the user to change where the note is  
"""
...
class UserDisplay:
    def __init__(self, master, cwidth=1024, cheight=768,
        ...
        **NOTE** -- Just because most of these arguments are keywords
        ...
        #This code adds a scrollbar to the scope window
        self.scy=Scrollbar(self.frame)
        self.scy.pack(side=RIGHT,fill=Y)
        self.scx=Scrollbar(self.frame,orient=HORIZONTAL)
        self.scx.pack(side=BOTTOM,fill=X)
        self._canvas.configure(xscrollcommand=self.scx.set,yscrollcommand=self.scy.set, scrollregion=(0,0,cwidth,cheight), height=cheight,width=cwidth)

        self.scx.config(command=self._canvas.xview)
        self.scy.config(command=self._canvas.yview)
    ...
    def note(self, msg):
        if msg is None or len(msg) == 0:
            ...
            if self.msg_win is None:				
                ...
                self.msg_win = self._canvas.create_window(5,25,window=f, anchor=NW)
    ...
    def note_at_location(self, msg, locX, locY):
        if msg is None or len(msg) == 0:
            if self.msg_win:
                self._canvas.delete(self.msg_win)
                self.msg_win = None
            ...
            if self.msg_win is None:				
                ...
                f = Frame(self._canvas, borderwidth=1)
                #Label(f, text='messages', bg='green').pack(expand=0, anchor=W)
                self.msg_win = self._canvas.create_window(locX,locY,
                                                      window=f, anchor=W)
                self.msg_label = Label(f, text=msg, font=font, justify=LEFT,									   bg=None, fg='black')
                self.msg_label.pack()
                self._placenote(locX, locY)
            else:
                self.msg_label.configure(text=msg)
...
""", fixed=1)
    ...
    def can2fb(self, x, y):
        ...
        if y is None:
            (x, y) = x
        hbarpos =  self.scx.get()
        #print x
        #print self._canvas.canvasx(x)
        #print y
        #print self._canvas.canvasy(y)
        newx = self._canvas.canvasx(x)
        newy = self._canvas.canvasy(y)
        return(newx - (self.w2), (self.h - newy) - self.h2)
