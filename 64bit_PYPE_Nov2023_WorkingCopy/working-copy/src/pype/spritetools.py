...
UnsignedInt8 --> np.uint8.   Nov 2023, Taekjun Kim
"""

import math
from numpy import *    # this is ugly, but I had to replace "from numeric import *"
import numpy as np; 
...
def pixelize(a, rgb=None, norm=1):
    """
    Convert a floating point array into an np.uint8 array suitable
...
def singrat(s, frequency, phase_deg, ori_deg, R=1.0, G=1.0, B=1.0):
    ...
    s.array[:] = transpose((array((R*i,G*i,B*i))+128).astype(np.uint8),
                           axes=[1,2,0])
