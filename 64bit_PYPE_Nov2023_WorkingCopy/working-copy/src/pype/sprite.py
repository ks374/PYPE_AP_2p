...
# $Id: sprite.py 119 2006-02-05 13:20:39Z mazer $
...
import types
import math
import copy as copymodule ### something shadows copy module
...
import numpy as np; 
Float64 = np.float64;    ### Feb 24, 2023 Taekjun
...
ALPHAMASKS = (16711680, 65280, 255, 0xff<<24)
...
class FrameBuffer:
        ...
        if maxbpp == 0:
            ...
            if fopengl:
                self.screen = pygame.display.set_mode((self.w, self.h),
                                                  flags,depth=32, vsync=1)   ## added vsync=1 Mar 2023
    ...
    def calcfps(self, duration=1000):
        ...
        **NOTE** --
        ...
        ### Need to find a way to remove stats.py ### Feb.2023. Taekjun
        ...
        if km == 0.0:
            ...
            #return 60 #for debugging - why is fps 10?
    ...
    def close(self):
        ...
        """
        ### In Python 3 exceptions module was removed and all standard exceptions 
        ### were moved to builtin module
        # import exceptions
        try:
            del self._sync_low
            del self._sync_high
    ...
    def string(self, x, y, s, color, flip=None, prefill=None, size=30):
        ...
        **NOTE** -- this requires the SDL_ttf package...
        ...
        if self.opengl:
            blitstr = pygame.image.tobytes(s, 'RGBA', 1)
    ...
    def rectangle(self, cx, cy, w, h, color, width=0):
        ...
        **width** -- 0 means fill the rectangle with the specfied color,
        ...
        if self.opengl:
            ...
            blitstr = pygame.image.tobytes(s, 'RGBA')
    ...
    **WARNING** --
        ...
        **NOTE** --
        ...
        if fname:
            ...
            try:
                ...
                self.userdict = copymodule.copy(image.userdict)
            except:
            ...
            #self.im = pygame.Surface((int(round(width)), int(round(height))), 0, 32)
            self.im = pygame.Surface((int(round(width)), int(round(height))), flags=pygame.SRCALPHA)            
            self.userdict = {}

        self.im = pygame.Surface.convert_alpha(self.im); 
    ...
    def asPhotoImage(self, alpha=None):
        ...
        **returns** -- PhotoImage represenation of the sprite's pixels.
        ...
        if alpha:
            ...
            im = self.im
        self.pil_im = Image.frombytes('RGBA', im.get_size(),
                                       pygame.image.tobytes(im, 'RGBA'))
    ...
    def noise(self, thresh=0.5):
        ...
        **returns** -- None
        ...
        self.array[:] = transpose(array([n, n, n]),
                                  axes=[1,2,0]).astype(np.uint8)[:]    # python3 corrected
    ...
    def circlefill(self, color, r=None, x=None, y=None, width=0):
        ...
        **NOTE** -- be careful about 'centerorigin'
        ...
        pygame.draw.circle(self.im, color, (int(round(x)), int(round(y))), int(round(r)), int(round(width)))


    def ellipse(self, color,w, h, x=None, y=None, width=0):
        """Draw a *filled* ellipse of specifed color -5-19-2006- Anitha

        **w, h** -- width and height of rect in pixels that the ellipse fills
        **x, y** -- ellipse center in world coords; defaults to center of sprite

        **width** -- pen width in pixels, if 0, then draw a filled ellipse

        **color** -- _C() parseable color spec
          
        **returns** -- None

        **NOTES** -- be careful about 'centerorigin'
        ...
        if x is None:
            ...
            x = self.X(x)-(w/2)
        #because pygame spec for ellipse is rect start point (not center)
        ...
        pygame.draw.ellipse(self.im, color, (x,y,w,h), width)


    def arc(self, color, w, h, x=None, y=None,ang_st=0, ang_end=2*pi, width=1):
        """Draw an arc of an ellipse of specifed color -5-19-2006- Anitha

        **w, h** -- width and height of rect in pixels that the ellipse fills
        **x, y** -- ellipse center in world coords; defaults to center of sprite

        **width** -- pen width in pixels > 0; default = 1
        **ang_st  -- Angle (radians) to start drawing ellipse(defaults to 0-right)
        **ang_end -- End angle - defaults to 2pi
        ...
        **NOTES** -- be careful about 'centerorigin'
        ...
        if x is None:
            ...
            x = self.X(x)-(w/2)
        #because pygame spec for arc is rect start point (not center)
        ...
        pygame.draw.arc(self.im, color, (x,y,w,h), ang_st, ang_end, width)          

    def polygon(self, color, verts, width=0):
        """Draw a polygon of specifed color -5-19-2006- Anitha

        **verts** -- polygon vertices in pixels; a list of x,y pairs

        **width** -- pen width in pixels, if 0, then draw a filled polygon
        ...
        **NOTES** -- be careful about 'centerorigin'
        ...
        for (x,y) in verts:
            x = self.X(x)
            y = self.Y(y)
            polyverts.append((x,y))
            
        pygame.draw.polygon(self.im, color, polyverts, width)          
    def unassumingpolygon(self, color, closed, verts, width):
        #uses pygame.lines instead of pygame.polygon
        #allows for 'unclosed polygons'
        #excuse the name, jkb.6/08
        ...
        pygame.draw.lines(self.im, color,closed, polyverts, width)

    def pdots(self,color,verts,radius): #last two params for debug
        color=_C(color, self.fb.opengl)  # color is a beast unto itself
        #segcolor=_C(segcolor)  # FOR DEBUGGING
        width= 0         # fills dots
        r= radius        # dot radius
        # perimeter dots drawn sequentially in a loop
        for (x,y) in verts:
            x=int(round(x))    # i think that all of these things drawn as int(must be rounded first)
            y=int(round(y))
            x = self.X(x) # ref frame change
            y = self.Y(y)
            pygame.draw.circle(self.im,color,(x,y),r,width)
## 		for (x,y) in segmentverts:   #FOR DEBUG
## 			x = self.X(x) # ref frame change
## 			y = self.Y(y)
## 			pygame.draw.circle(self.im,segcolor,(x,y),r,width)	

    def tlines(self,color,line_coords,width):
        #seems like what i want to do is package list of coordinates into paired list.
        #then i can step through in a for loop and say for ([x1,y1],[x2,y2]) in verts
    ...
    def rotate(self, angle, preserve_size=1, trim=0):
        ...
        **NOTES** --
        ...
        #self.im = new.convert(ALPHAMASKS)
        self.im = pygame.Surface.convert_alpha(new)
    ...
    def alpha_aperture(self, r, x=0, y=0):
        ...
        **returns** -- None
        ...
        d = where(less((((self.ax-x)**2)+((self.ay-y)**2))**0.5, r),
                  255, 0).astype(np.uint8)   # python3 corrected
        self.alpha[:] = d
    ...
    def alpha_gradient(self, r1, r2, x=0, y=0):
        ...
        **returns** -- None
        ...
        d = where(less(d, 0), 0,
                  where(greater(d, 255), 255, d)).astype(np.uint8)   # python3 corrected
        self.alpha[:] = d
    ...
    def alpha_gradient2(self, r1, r2, bg, x=0, y=0):
        ...
        **returns** -- None
        ...
        #alpha1 = clip(d, 0.0, 1.0)

        #### trying to make this more sigmodal
        d = hypot(self.ax-x, (self.ay-y))
        k = 4.0/(r2-r1) #this is the slope
        t0 = (r1+r2)/3.0 ## offset
        alpha = 1.0 - (1.0/(1+e**(-(d-t0)*k)))
        ...
        i[:] = ((alpha * i.astype(float)) +
                ((1.0-alpha) * bgi)).astype(np.uint8)   # python3 corrected
        self.alpha[:] = 255;

    
    def alpha_gradient_ellipse(self, r1, r3, bg, x=0, y=0):
        """Linear gradient vignette
        ...
        **r1,r3** -- inner radii in pixels along the two axes
        ...
        **returns** -- None
        """
        r2 = r1*1.5 # completely fades out at 1.5 times inner rad

        #### trying to make this more sigmodal
        d = hypot(self.ax-x, (self.ay-y)*r1/r3)
        k = 4.0/(r2-r1) #this is the slope
        t0 = (r1+r2)/3.0 ## offset
        alpha = 1.0 - (1.0/(1+e**(-(d-t0)*k)))
        i = pygame.surfarray.pixels3d(self.im)
    ...
    def fuzzy_image(self, pixwid = 10):
        """This is a simple way to blur an image. Faster than doing 
        pixel based gaussian blurs. Pixwid is the number of pixels to 
        average along the positive and negative axes. pixwid =10 means
        each pixel the average of 21 pixels including itself.
        --- Anitha 9/9/09
        """
        #compute weighting factor
        const = 1;
        sg = float(pixwid)
        for i in range(1,pixwid+1):
            const += 4*exp(-float((i*i)/(2*sg*sg))) + 4*exp(-float((2*i*i)/(2*sg*sg))) 
        print(const)	
        pixs = pygame.surfarray.pixels3d(self.im)
        soften = array(pixs)
        soften = soften.astype(Float)/float(const)
        for i in range(1,pixwid+1):
            c1 = exp(-float((i*i)/(2*sg*sg)))/const
            c2 = exp(-float((2*i*i)/(2*sg*sg)))/const
            soften[i:,:]+=pixs[:-i,:].astype(Float)*c1
            soften[:-i,:]+=pixs[i:,:].astype(Float)*c1
            soften[:,:-i]+=pixs[:,i:].astype(Float)*c1
            soften[:,i:]+=pixs[:,:-i].astype(Float)*c1
            #Diagonal terms
            soften[i:,i:]+=pixs[:-i,:-i].astype(Float)*c2
            soften[:-i,:-i]+=pixs[i:,i:].astype(Float)*c2
            soften[i:,:-i]+=pixs[:-i,i:].astype(Float)*c2
            soften[:-i,i:]+=pixs[i:,:-i].astype(Float)*c2
        pixs[:] = soften.astype(np.uint8)   # python3 corrected
    ...
    def dim(self, mult, meanval=128.0):
        ...
        **NOTE** --
        ...
        pixs[:] = (float(meanval) + ((1.0-mult) * \
               (pixs.astype(Float)-float(meanval)))).astype(np.uint8)   # python3 corrected
    ...
    def thresh(self, t):
        ...
        **returns** -- None
        """
        pixs = pygame.surfarray.pixels3d(self.im)
        pixs[:] = where(less(pixs, t), 1, 255).astype(np.uint8)   # python3 corrected
    ...
    def blit(self, x=None, y=None, fb=None, flip=None, force=0, fast=None):
        ...
        **NOTE** --
        ...
        if fb.opengl:
            ...
            #blitstr = pygame.image.tobytes(self.im, 'RGBA', 1)           # for 32 bit maxbpp
            if hasattr(self, 'blitstr'):
                blitstr = self.blitstr
            else:
                blitstr = pygame.image.tobytes(self.im, 'RGBA_PREMULT', 1)   # for 24 bit maxbpp
            #blitstr = np.dstack((pygame.surfarray.array3d(self.im),
            #                     pygame.surfarray.array_alpha(self.im))); 
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, self.w, self.h,
                         0, GL_RGBA, GL_UNSIGNED_BYTE, blitstr)

            ic = ((0,0), (1,0), (1,1), (0,1))
            oc = ((scx, scy), (scx+self.w, scy), 
                  (scx+self.w, scy+self.h), (scx, scy+self.h)) 
            glBegin(GL_QUADS)                              
            for n in range(4):
                glTexCoord2f(ic[n][0], ic[n][1])
                glVertex(oc[n][0], oc[n][1])
            glEnd()
            ...
            del self.alpha
            del self.array
            self.im.unlock() ####added unlock AP May-31-2006
    ...
    def render(self):
        ...
        **NOTE** --
        ...
        """
        if self.fb.opengl:
            self.fastim = pygame.image.tobytes(self.im,'RGBA', 1)
        else:
    ...
    def clone(self):
        ...
        s.alpha[:] = self.alpha[:]
        s.userdict = copymodule.copy(self.userdict)
