...
{
  int bits = 0;
  int i, success, check, sentbyte, ct = 0, strobe_high_time_ms = 2;
...
//      success = comedi_dio_bitfield(comedi_dev,dig_o,ISA_WRITEMASK,&bits);
    ...
    if (use8255) {
      bits = bits<<BANK_C;
      check = bits;
      success = Eth32_write_byte(eth32_handle, 1, sentbyte|0x80); // set strobe bit
...
//      fprintf(stderr, "Writing %X to comedi with %X mask\n", bits, PCI_STRWRITEMASK);
    ...
    if (use8255) {
      bits = bits<<BANK_C;
      success = Eth32_write_byte(eth32_handle, 1, sentbyte&0x7F); // clear strobe bit
