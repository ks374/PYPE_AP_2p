...
**   configured port C for output and added support in dig_out to 
**   write to port C as well
**
**
*/
#include <unistd.h>
...
#include <eth32.h> // edited May 2019, Taekjun
#include "Eth32_interface.c"
...
// bank C (16-23)is also configured as output -- Anitha
...
#define PCI_WRITEMASK   (255<<BANK_B)|(127<<BANK_C) //this is for B and C output
#define PCI_STRWRITEMASK 128<<BANK_C // this is just for writing the strobe bit (strobe 
...
static int comedi_init()
{
  ...
  fprintf(stderr, "In comedi_init\n");  
  if (!(eth32_handle = Eth32_init())){
  // quick test...: Eth32_write_byte(eth32_handle, 0, 0xAA);
  ...
  if (use8255) {
    ...
    // also configuring PORT C as output
    if (comedi_dio_config(comedi_dev,dig_io,BANK_A,COMEDI_INPUT) &&
	comedi_dio_config(comedi_dev,dig_io,BANK_B,COMEDI_OUTPUT) &&
	//Port C is two banks of 4 and so ...
	comedi_dio_config(comedi_dev,dig_io,BANK_C,COMEDI_OUTPUT)&&
	comedi_dio_config(comedi_dev,dig_io,BANK_C+4, COMEDI_OUTPUT)) {
      return(1);
    } else {
      return(0);
    }
...
{
  lsampl_t sample = 0;
  // int success; //Commented out. 11/15
...
static void dig_in()
{
  ...
  int i, bits=0, last; //Delete success. Changed 11/14. 
  // Not sure why bits is defined here ans used below with out initialization. It's not =0 as other functions. But not sure whether it's correct. CHanged 11/14. 
...
//this is for writing strobed words
static void dig_str_out()
{
  ...
  if (dummymode) {
    ...
    // Now for port C --- Anitha
     for (i = 0; i < 8 && i < NDIGOUTC; i++) {
      bits = bits | (dacq_data->doutc[i] << (i+NDIGOUT));
    }
...
//      success = comedi_dio_bitfield(comedi_dev,dig_o,ISA_WRITEMASK,&bits);
    ...
    // this is the strobe bit
    i = 7;
    bits = 1<<i;
    if (use8255) {
      bits = bits<<BANK_C;
      //check = bits; //Delete, not used. Changed 11/14
      //success = Eth32_write_byte(eth32_handle, 1, sentbyte|0x80); // set strobe bit //Delete, not used. Changed 11/14
      
//      fprintf(stderr, "Strobe high: \nWriting %X to Eth32 Port 1\n", sentbyte|0x80);
//      success = comedi_dio_bitfield(comedi_dev,dig_io,PCI_STRWRITEMASK,&bits);
//      fprintf(stderr, "Writing %X to comedi with %X mask\n", bits, PCI_STRWRITEMASK);
      clock_gettime(CLOCK_REALTIME, &ts);
      t0 = ts.tv_nsec;
    }
    ...
    if (use8255) {
      bits = bits<<BANK_C;
      //success = Eth32_write_byte(eth32_handle, 1, sentbyte&0x7F); // clear strobe bit //Delete, not used. Changed 11/14
//      fprintf(stderr, "Strobe low: \nWriting %X to Eth32 Port 1\n", sentbyte&0x7F);
//      success = comedi_dio_bitfield(comedi_dev,dig_io,PCI_STRWRITEMASK,&bits);
//      fprintf(stderr, "Writing %X to comedi with %X mask\n", bits, PCI_STRWRITEMASK);      
      Eth32_verify(eth32_handle);
    }
