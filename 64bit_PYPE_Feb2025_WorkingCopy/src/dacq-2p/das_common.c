...
static void openiris_halt()
{
  ...
  if (tracker_mode == OPENIRIS) {
    ...
    LOCK(semid);
    dacq_data->openiris_terminate = 1; /*What kind of genius write this so that you terminate the flag and then check the server status...*/
    UNLOCK(semid);
    do { 
      LOCK(semid);
      k = dacq_data->openiris_ready;
...
static void mainloop(void)
{
  ...
  do {
	...
	tx = ty = tp = -1;
      ...
      y = dacq_data->adc[1];
      UNLOCK(semid);
    } else if (tracker_mode == OPENIRIS){
      LOCK(semid);
      x = dacq_data->openiris_x;
      y = dacq_data->openiris_y;
      UNLOCK(semid);
      pa = -1; /*Not use pa (pupil area information)*/
    } else {
      x = dacq_data->adc[0];
      y = dacq_data->adc[1];
      pa = -1;
  ...
  iscan_halt();
  eyelink_halt();
  openiris_halt(); //They only halt the flag, not the actual thread. 
