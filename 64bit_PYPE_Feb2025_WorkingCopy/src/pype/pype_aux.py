...
class Logfile:
    ...
    def write(self, line):
        if self.filename:
            f = open(self.filename, "ab")  # a-->ab python3 related
            f.write(line)
            f.close()
...
def param_expand(s, integer=None):
    ...
    **returns** -- float or integer matching parameter string
    ...
    if len(l) > 1:
        if len(l) == 3:
            start, stop, step = list(map(int, l))
        elif len(l) == 2:
            start, stop = list(map(int, l))
            step = 1
...
def open_dumpfile(fname, mode, header=None):
    ...
    if mode[0] == 'rb':   # r-->rb python3 related
        f = open(fname, mode)
        header = ''
        while 1:
            c = f.read(1)
            if c == '\014':
                ...
                header = header + c
        return f, header
    elif mode[1] == 'wb':  # w-->wb python3 related
        f = open(fname, mode)
        if header:
            f.write(header + '\014')
        return f
...
def labeled_dump(label, obj, f, bin=0):
    ...
    f.write(b'<<<%s>>>\n' % label.encode())     ### for python3 compatibility
    dump(obj, f, bin)
...
def labeled_load(f):
    ...
    while 1:
        l = f.readline().decode()     ### for python3 compatibility
        if not l:
            return None, None
        elif l[:3] == '<<<' and l[-4:] == '>>>\n':
            return l[3:-4], load(f)
...
def p_info(app, msg=None, color='black', nl=1):    ### change function name. Mar 2023
...
def showparams(app, P, clearfirst=1):
    ...
    **P** -- parameter dictionary
    ...
    See p_info() function, same module. # change function name. Mar 2023
    """
    if clearfirst:
        p_info(app)   # change function name. Mar 2023
    keys = list(P.keys())
    keys.sort()
    n = 0
    while n < len(keys):
        ...
        while n < len(keys) and len(s) < 25:
            ...
            n = n + 1
        p_info(app, s) # change function name. Mar 2023
