...
Sun Jul 30 6:14:pm 2006 Anitha

- added encode_plex(self, code) to output storbed words for output of
  storbed words for behavioral codes and stimulus numbers.

Fri Aug 18 14:05:22 2006 Anitha

- Added functionality to use eyelink calibrated output. Eyelink built-in 
  calibration can be used and the pype offset and gain can be set to 0 and
  1 respectively. EYELINK_OUT is now an entry in the Config file. 
  Default is RAW; can also use CALIBRATED

Mon Nov 26 16:37:00 2007 Phil

- added udpy_note_at_location to pype class to make use of new method in userdpy.py

March 18, 2008 Anitha
- added eyelink_start_file and eyelink_end_file to facilitate opening and
closing EDF files from TASK files. But its not working yet.
...
class PypeApp:
    ...
    def __init__(self, framebuffer=1, gui=1):
        ...
        # Anitha added 18-aug-2006:
        # this to facilitate using eyelink calibrated output (instead of raw)
        # setting this as an environ var
        ...
        os.environ['ETH32_IP'] = self.config.get('ETH32_IP') 
        os.environ['ETH32_PORT0_DIR'] = self.config.get('ETH32_PORT0_DIR') 
        os.environ['ETH32_PORT1_DIR'] = self.config.get('ETH32_PORT1_DIR')
        os.environ['ETH32_PORT2_DIR'] = self.config.get('ETH32_PORT2_DIR')
        ...
        if not gui:
            ...
            (
                ...
                ("pause_color",	"(128,128,128)",is_color),
        ...
        # Anitha Aug 18, 2006 - added gaze output for calibrated data
        eyelink_opts = eyelink_opts + ':link_sample_data = PUPIL,GAZE,AREA'
    ...
    def tally(self, type=None, clear=None):
        ...
        s = ''
        for k in list(self.tallycount.keys()):
            if len(s) == 0:
                s = 'Press to Clear: '
    ...
    def eyeset(self):
        ...
        # Anitha Aug 18 2006 - Do this only in the raw mode; calibrated mode
        # already outputs in appropriate resolution from EYELINK
        if self.config.get('EYELINK_OUT') == 'RAW':
    ...
    def _savestate(self):
        ...
        if self.tk:
            ...
            file = open(self._statefile(), 'wb') # edit for python3
            pickle.dump(d, file)
            file.close()
    ...
    def _readstate(self):
        ...
        try:
            ...
            file = open(self._statefile(), 'rb') # edit for python3
            d = pickle.load(file)
            file.close()
            return d
        except IOError:
    ...
    def idlefn(self, ms=None, update=1, toplevel=None, fast=None):
        ...
        To facilitate keys for specific tasks (without collision across
        tasks) added functionality to pop back to keyque if its not one
        of the standard keys -- Anitha Aug 14 2006
        ...
        *****************************************************
        ...
        if self.tk is None:
            ...
            if c == 'F1':				
                ...
                self.keyque._push(ev)
                #If not one of the standard keys push back on queue to be
                #retrieved by task program -AP Aug 14/2006
    ...
    def reward(self, multiplier=1.0, dobeep=1, dojuice=1):
        """
        Deliver a squirt of juice based on the dropsize slider and
        the current state of the reward schedule settings
        Jamie has dobeep setup for isolated secondary reinforcement from computer
        I'm using it for isolated hardware reinforcement --- Anitha (Dec 4, 2008)
        dojuice is an option to give juice or not but default is to give juice as
        normal --- Anitha Dec 5th,2008
        """
        ...
        if var > 0:
            ...
            if not self.config.iget('DACQ_TESTMODE'):
                _thread.start_new_thread(self.__reward2, (t,dobeep,dojuice))
    ...
    def __reward2(self, t, dobeep,dojuice):
        """
        ...
        self.juice_drip(t,dobeep,dojuice)
        self._rewardlock.release()

    def juice_on(self,dobeep=1,dojuice=1):
        if self.pport:
            pp_juice(1)
        else:
            dacq_juice(1,dobeep,dojuice)
            
    def juice_off(self, dobeep=0,dojuice=0):
        if self.pport:
            pp_juice(0)
        else:
            dacq_juice(0,dobeep,dojuice)

    def juice_drip(self, ms, dobeep,dojuice):
        if self.pport:
            pp_juice_drip(int(0.5+ms))
        else:
            dacq_juice_drip(int(0.5+ms), dobeep,dojuice)
    ...
    def close(self):
        ...
        #import exceptions    ## In Python 3 exceptions module was removed 
                              ## and all standard exceptions were moved to builtin module
        self.unloadtask()
    ...
    def encode(self, code):
        ...
        if isinstance(code, tuple):   ### edit for python3         
            for c in code:
    ...
    def encode_plex(self, code):
        """
        Send behavioral <code> to plexon with the current
        trial timestamp (relative to the last call to start_trial.
        Same as encode but then sends a strobed word to plexon
    
        Sun Jul 30 2006 Anitha
          Changed to output a strobed word to dacq
        """
        import time as T
        t = dacq_ts()
        if not self.config.iget('DACQ_TESTMODE'):
        #this is causing a wedge!!!
            dacq_str_word(pype_plex_code_dict(code))## Write the strobed word
        return t

    #def eyelink_start_file(self, filename):
    #	"""Open an eyelink data file March 17, 2008 Anitha
    #	"""
    #	dacq_open_eyefile(filename)
    
    #def eyelink_end_file(self):
    #	dacq_close_eyefile()
    ...
    def record_write(self, result=None, rt=None, pdict=None, taskinfo=None):
        ...
        if isinstance(taskinfo, tuple)==0:   # edit for python3
            taskinfo = (taskinfo,)
        ...
        if self.record_file:
            ...
            rec = [ENCODE, info, self.record_buffer,
                   ...
                   # as p0 and s0
            f = open(self.record_file, 'ab')   # a-->ab. for python3
...
class FixWin:
    ...
    def genint(self, enable, n=0):
        if enable:
            dacq_fixwin_genint(n, 1)
        else:
            dacq_fixwin_genint(n, 0)
            # also: clear saved flag -- no more interupts
            # in theory you could loose a pending interupt here, but it
            # should be unlikely.
            self._post_fixbreak = 0
...
class Timer:
    ...
    def reset(self):
        self._start_at = int(dacq_ts())

    def ms(self):
        return int(dacq_ts()) - self._start_at

    def us(self):
        sys.stderr.write("Warning: timer.us method obsolete, faking it\n");
        return 1000 * (int(dacq_ts()) - self._start_at)
...
def candy2(app):
    ...
    if app._candy:
        ...
        try:
            f = open(pyperc('candy.lst'), 'rb')   #r-->rb for python3
            l = f.readlines()
            f.close()
        except IOError:
            warn('error',
...
def appbusy(t, busyflag=1):
    if busyflag:
        ...
        t.configure(cursor='')
    if len(list(t.children.keys())) > 0:
        for name in list(t.children.keys()):
